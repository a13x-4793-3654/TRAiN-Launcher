...
- プロファイルごとに専用のゲームディレクトリ(`gameDir`)を保持し、Mod / リソースパック / セーブデータ / `options.txt` は他のプロファイルと混ざらないよう分離して管理する(バージョン本体・ライブラリ・アセットは引き続き共有し、二重ダウンロードを避ける)。作成/更新時に `gameDir` を明示的に指定していない場合は、TRAiN Launcherがプロファイル専用のフォルダを自動的に割り当てる(異なるMinecraftバージョン/Modローダーのプロファイル間でMod・リソースパックが混在して起動できなくなることを防ぐため)。公式 Minecraft Launcher と共有する `.minecraft` フォルダを明示的に使いたい場合のみ、そのパスを `gameDir` に指定する。この `gameDir` は公式 Minecraft Launcher の `launcher_profiles.json` の `gameDir` と双方向に同期される
- TRAiN サーバーへの参加時に自動作成/更新されるプロファイルも同様にサーバーごとに専用のゲームディレクトリを自動的に割り当てるため、複数のTRAiNサーバー間でMod構成が競合しない

### ゲーム設定の取り込み / バックアップ

- プロファイルの「設定を取り込む」から、同じ Minecraft バージョンの別プロファイルを選び、キー割り当て・音量・描画・言語などの `options.txt` の標準設定を取り込めます。公式ランチャー由来のプロファイルも対象です。移行先のリソースパック選択・互換性確認・直近の接続先は維持し、Mod の設定ファイル、ワールド、Mod / リソースパック本体はコピーしません
- バージョン比較はローカルの `inheritsFrom` をたどり、Fabric / Forge 等のローダーバージョンではなく基になる Minecraft バージョンで行います。`latest-release` / `latest-snapshot` のようにバージョンが確定しない指定は、具体的なバージョンへ変更してください。両方の `options.txt` に形式バージョンがあり異なる場合も、誤った移行を避けるため取り込みを停止します
- 専用ゲームフォルダーが未割り当ての古いTRAiNプロファイルを移行先にする場合は、先にプロファイル画面で一度編集・保存してください。設定の取り込みやバックアップ作成では、元のゲームフォルダーやModファイルを移動しません
- 独立した「バックアップ」画面で、保存元・復元先のプロファイルと「標準設定のみ」「ゲームデータ全体」を選べます。バックアップはランチャー設定ディレクトリ配下の `backups` (Windows: `%APPDATA%\train-launcher\backups`) に保存します。「フォルダーを開く」から外部ストレージへのコピーも可能です
- 全体バックアップは設定・ワールド・Mod・リソースパック・シェーダー・Mod固有データを保存します。公式ランチャーの認証情報・プロファイル、共有の `assets` / `libraries` / `versions` / `runtime`、ログ類は対象外で、全体復元時もこれらを変更しません。復元は同じ Minecraft バージョンに限定し、全体復元では Mod ローダーも一致する必要があります
- 設定の取り込み・復元の前に、既存の対象データがあれば自動退避バックアップを作成します。標準設定がまだなく、リソースパック選択など保持対象の項目しかない場合は、自動退避せずにそれらを維持して取り込み・復元します。全体復元ではバックアップ時点に戻すため、その後追加したゲームデータも置き換わります。ファイル操作または管理情報の保存に失敗した場合は元のデータへ戻し、復旧できなかった場合は復旧用データを保持してエラーを表示します。中断された全体復元があるフォルダーでは起動を停止するので、「バックアップ」から全体復元を完了してください
- ランチャーから起動・準備中の Minecraft がある間は、取り込み・バックアップ・復元やゲームファイルの変更を停止します。公式ランチャー等から起動した Minecraft は検出できないため、それらも終了してから操作してください。同じゲームフォルダーを共有するプロファイルには変更が共通で反映されます
- サーバーから起動しても、標準設定、Java パス、最大メモリ、専用ゲームフォルダーは保持します。サーバーが管理する Minecraft バージョン、Mod / リソースパック構成は引き続き起動時に同期します。全体復元後も次回起動でサーバーの配布構成へ同期されます
